...
const MyBookings = (props) => {
  ...
  const fetchBookings = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/book/getallbookings', {
        method: 'GET',
        headers: {
          "auth-token": token
        }
  ...
  const handleCancelBooking = async (bookingId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/book/bookings/${bookingId}`, {
        method: 'DELETE',
        headers: {
  ...
  const handleReceipt = async (bookingId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/book/receipt/${bookingId}`, {
        method: 'GET',
        headers: {
          "auth-token": localStorage.getItem('token')
        }
